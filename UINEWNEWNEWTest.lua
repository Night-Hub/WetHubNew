...
function UILibrary.LoadConfig(name, options)
	...
	local autofillMissing = (options.autofillMissing == nil) and true or (options.autofillMissing == true)
	local saveIfPatched = (options.saveIfPatched == nil) and true or (options.saveIfPatched == true)

	if typeof(readfile) ~= "function" then
		warn("UILibrary.LoadConfig: readfile() not available.")
		return false
	end
	...
	--// patch missing keys so configs never "break" when you add new UI
	local patched = false
	if autofillMissing then
		for key, handle in pairs(UILibrary.__Registry) do
			if cfg.Values[key] == nil then
				...
				--// requested rule: missing toggle key -> false
	...
	--// apply
	UILibrary.ApplyConfig(cfg, fireCallbacks)

	--// rewrite config if we patched it
...
--// list config files (requires listfiles)
...
function UILibrary.Load(GUITitle)
	...
	function TabLibrary.AddPage(PageTitle, SearchBarIncluded)
		...
		function PageLibrary.AddDropdown(Text, ConfigurationArray, Callback)
			...
			--// handle
			local key = _makeKey(PageTitle, "Dropdown", Text)
			local Handle = {}
		...
		function PageLibrary.AddSlider(Text, ConfigurationDictionary, Callback, Parent)
			...
			local key = _makeKey(PageTitle, "Slider", Text)
			local Handle = {}
		...
		function PageLibrary.AddColourPicker(Text, DefaultColour, Callback)
			...
			local key = _makeKey(PageTitle, "ColourPicker", Text)
			local Handle = {}
		...
		function PageLibrary.AddValueButton(Text, Default, Options, OnClick, Getter)
			...
			local key = _makeKey(PageTitle, "ValueButton", Text)
			local Handle = {}
